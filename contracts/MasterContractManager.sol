...
contract MasterContractManager is BoringOwnable, BoringFactory {
    ...
    function _calculateDomainSeparator(uint256 chainId) private view returns (bytes32) {
        return keccak256(abi.encode(DOMAIN_SEPARATOR_SIGNATURE_HASH, keccak256("Alpine V1"), chainId, address(this)));
    }
    ...
    /// @notice Other contracts need to register with this master contract so that users can approve them for the Alpine.
                    ...
                    abi.encodePacked(
                        ...
                        keccak256(
                            abi.encode(
                                ...
                                approved
                                    ? keccak256("Give FULL access to funds in (and approved to) Alpine?")
                                    : keccak256("Revoke access to Alpine?"),
                                user,
                                masterContract,
                                approved,
                                nonces[user]++
