...
contract Salary is BoringBatchable {
    ...
    constructor(Alpine _alPine) public {
        alPine = _alPine;
        _alPine.registerProtocol();
    }

    // Included to be able to approve Alpine and create in the same transaction (using batch)
    function setAlpineApproval(
        address user,
        bool approved,
        uint8 v,
        bytes32 r,
        ...
        alPine.setMasterContractApproval(user, address(this), approved, v, r, s);
    }
    ...
    uint8 private constant MODE_ALP = 0; // Use Alpine balance
    uint8 private constant MODE_ERC20_SKIM = 1; // Use ERC20 tokens deposited onto the Alpine contract
    ...
    function create(
        ...
        if (mode == MODE_ALP) {
            // Fund this salary using the funder's Alpine balance. Convert the amoutn to shares, then transfer the shares
            shares = alPine.toShare(token, amount, false);
            alPine.transfer(token, msg.sender, address(this), shares);
        } else {
            ...
            (, shares) = alPine.deposit(token, mode == MODE_ERC20_SKIM ? address(alPine) : msg.sender, address(this), amount, 0);
        }
    ...
    function withdraw(
        ...
        if (toAlpine) {
            alPine.transfer(salary.token, address(this), to, pendingShares);
        } else {
            alPine.withdraw(salary.token, address(this), to, 0, pendingShares);
        }
    ...
    function cancel(
        ...
        if (toAlpine) {
            alPine.transfer(salaries[salaryId].token, address(this), to, sharesLeft);
        } else {
            alPine.withdraw(salaries[salaryId].token, address(this), to, 0, sharesLeft);
        }
