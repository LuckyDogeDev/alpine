...
contract CompoundStrategy is IStrategy, BoringOwnable {
    ...
    modifier onlyAlpine {
        // Only the alpine can call harvest on this strategy
        require(msg.sender == alpine, "CompoundStrategy: only alp");
    ...
    function skim(uint256 amount) external override onlyAlpine {
        require(cToken.mint(amount) == 0, "CompoundStrategy: mint error");
    }

    // Harvest any profits made converted to the asset and pass them to the caller
    /// @inheritdoc IStrategy
    function harvest(uint256 balance, address sender) external override onlyAlpine returns (int256 amountAdded) {
        ...
        // Transfer the profit to the alpine, the amountAdded at this point matches the amount transferred
        token.safeTransfer(alpine, amountAdded_);
    ...
    function withdraw(uint256 amount) external override onlyAlpine returns (uint256 actualAmount) {
        ...
        token.safeTransfer(alpine, actualAmount);
    }
    ...
    function exit(uint256 balance) external override onlyAlpine returns (int256 amountAdded) {
        ...
        // Transfer all tokens to alpine
        token.safeTransfer(alpine, amount);
