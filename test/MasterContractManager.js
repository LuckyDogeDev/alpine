...
describe("MasterContractManager", function () {
    ...
    describe("Master Contract Approved", function () {
        it("Returns false for pair which has not been set", async function () {
            expect(await this.alPine.masterContractApproved(this.masterContractMock.address, this.alice.address)).to.be.false
        })
        ...
        it("Returns true for pair which has been set", async function () {
            ...
            expect(await this.alPine.masterContractApproved(this.masterContractMock.address, this.carol.address)).to.be.true
        })
    })

    describe("whitelist Master Contract", function () {
        it("Reverts if caller is not the owner", async function () {
            await expect(this.alPine.connect(this.bob).whitelistMasterContract(this.masterContractMock.address, true)).to.be.revertedWith(
                "Ownable: caller is not the owner"
            )
        })

        it("Reverts if whitelisting address(0) as MasterContract", async function () {
            await expect(this.alPine.connect(this.alice).whitelistMasterContract(ADDRESS_ZERO, true)).to.be.revertedWith(
                "MasterCMgr: Cannot approve 0"
            )
            expect(await this.alPine.whitelistedMasterContracts(ADDRESS_ZERO)).to.be.false
        })

        it("Allows to WhiteList MasterContract", async function () {
            await this.alPine.connect(this.alice).whitelistMasterContract(this.masterContractMock.address, true)
            expect(await this.alPine.whitelistedMasterContracts(this.masterContractMock.address)).to.be.true
        })
    })
    ...
    describe("Set Master Contract Approval with WhiteList", function () {
        it("Reverts with address zero as masterContract", async function () {
            await expect(setMasterContractApproval(this.alPine, this.carol, this.carol, "", ADDRESS_ZERO, true, true)).to.be.revertedWith(
        ...
        it("Reverts with non whiteListed master contract", async function () {
            await expect(
                setMasterContractApproval(this.alPine, this.carol, this.carol, "", "0x0000000000000000000000000000000000000001", true, true)
        ...
        it("Reverts with user not equal to sender", async function () {
            ...
            await expect(
                setMasterContractApproval(this.alPine, this.carol, this.alice, "", this.masterContractMock.address, true, true)
            ).to.be.revertedWith("MasterCMgr: user not sender")
        })

        it("Reverts with contract being a clone", async function () {
            const deployTx = await this.alPine.deploy(this.badMaster.address, "0x", true)
            ...
            await expect(badMasterClone.attack(this.alPine.address)).to.be.revertedWith("MasterCMgr: user is clone")
        })

        it("Emits LogSetMasterContractApproval event with correct arguments", async function () {
            await this.alPine.whitelistMasterContract(this.masterContractMock.address, true)
            await expect(setMasterContractApproval(this.alPine, this.alice, this.alice, "", this.masterContractMock.address, true, true))
                .to.emit(this.alPine, "LogSetMasterContractApproval")
    ...
    describe("setMasterContractApproval with Permit", function () {
        it("Reverts with address zero as user", async function () {
            ...
            await expect(
                this.alPine.setMasterContractApproval(
        ...
        it("Reverts if signature is incorrect", async function () {
            await expect(
                setMasterContractApproval(this.alPine, this.bob, this.bob, this.carolPrivateKey, this.masterContractMock.address, true, false)
